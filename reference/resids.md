...
- object:
  ...
  [`VGAM::vglm()`](https://rdrr.io/pkg/VGAM/man/vglm.html). Or a matrix
  of fitted probabilities (e.g., from a multinomial model).
...
- y:
  ...
  A vector of the response variable (only used when `object` is a matrix
  of probabilities).
